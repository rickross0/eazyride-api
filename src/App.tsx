...
function ProgressBar() {
  ...
  return (
    <div className="fixed bottom-6 left-6 right-6 bg-navy-900/80 backdrop-blur-xl rounded-full h-2 z-40 overflow-hidden">
      <motion.div
        className="h-full bg-gradient-to-r from-gold to-cyan glow-gold"
    ...
    <div className="fixed top-20 right-6 z-40 text-center">
      <div className="bg-navy-900/80 backdrop-blur-xl rounded-2xl p-3 border border-gold/30">
        ...
        <div className="w-3 h-24 bg-navy-700 rounded-full overflow-hidden relative">
          <motion.div
            className="absolute bottom-0 w-full bg-gradient-to-t from-gold to-cyan"
            animate={{ height: `${fill}%` }}
            transition={{ ease: 'easeOut' }}
          />
        </div>
...
function Quiz({ lang }: { lang: string }) {
  ...
  const questions = [
    {
      q: lang === 'ar' ? 'هل تحتاج رحلة سريعة؟' : lang === 'so' ? 'Ma u baahantahay raac degdeg ah?' : 'Need a quick ride?',
      a: 'rider'
    },
    {
      q: lang === 'ar' ? 'هل تريد كسب المال بالقيادة؟' : lang === 'so' ? 'Ma rabtaa inaad kastumo ka dhigto baabuur wadida?' : 'Want to earn by driving?',
      a: 'driver'
    },
    {
      q: lang === 'ar' ? 'هل لديك مطعم تريد توسيعه؟' : lang === 'so' ? 'Ma leedahay maqaayad aad u ballaariso?' : 'Have a restaurant to expand?',
      a: 'store'
    },
    {
      q: lang === 'ar' ? 'هل تدير أسطول سيارات؟' : lang === 'so' ? 'Ma maareynaysaa flit gaadiid ah?' : 'Manage a car fleet?',
      a: 'provider'
    }
  ];
  ...
  return (
    <>
      <motion.button
        onClick={() => { setOpen(true); setStep(0); setScore(0); }}
      ...
      {open && (
        ...
        >
          <motion.div
            ...
            className="bg-navy-900 p-10 rounded-3xl max-w-md w-full text-white border border-gold/30 shadow-2xl"
          >
            <button onClick={() => setOpen(false)} className="absolute top-4 right-4 text-white/60 hover:text-white cursor-pointer">
              <X className="w-6 h-6" />
            </button>
            ...
            {!isDone ? (
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <p className="text-2xl font-bold text-center mb-6">
                  {lang === 'ar' ? 'التطبيق المثالي لك:' : lang === 'so' ? 'App-ka kugu habboon:' : 'Your perfect app:'}
                </p>
                ...
                <a href="#downloads">
                  <button onClick={() => setOpen(false)} className="w-full bg-gradient-to-r from-gold to-cyan text-navy-900 p-4 rounded-xl font-bold cursor-pointer hover:opacity-90 transition-opacity">
                    ...
                    {lang === 'ar' ? 'حمل الآن' : lang === 'so' ? 'Soo Degso Hadda' : 'Download Now'}
                  </button>
                </a>
              </motion.div>
            )}
      ...
      {show && (
        ...
        >
          <motion.div
            ...
            className="bg-navy-800 border border-gold/30 rounded-3xl max-w-md w-full p-8 shadow-2xl text-white"
          >
            ...
            <h2 className="text-2xl font-bold text-gold mb-4">
              {lang === 'ar' ? '🚀 انضم لثورة النقل!' : lang === 'so' ? '🚀 Ku biir kacaanka gaadiidka!' : '🚀 Join the Ride Revolution!'}
            </h2>
            {!submitted ? (
              <>
                <p className="text-white/80 mb-6">
                  {lang === 'ar' ? 'احصل على آخر الأخبار والعروض.' : lang === 'so' ? 'Hel wararka ugu dambeeya iyo dalabyada.' : 'Get the latest news & deals.'}
                </p>
                <div className="flex gap-2">
                  ...
                  />
                  <button onClick={handleSubmit} className="bg-gradient-to-r from-gold to-cyan text-navy-900 font-bold px-6 py-3 rounded-xl cursor-pointer hover:opacity-90 transition-opacity">
                    {lang === 'ar' ? 'إرسال' : lang === 'so' ? 'Dir' : 'Go'}
                  </button>
                </div>
              </>
            ) : (
              <motion.p initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="text-gold text-xl font-bold text-center">
                {lang === 'ar' ? '🎉 شكراً! سنتواصل قريباً.' : lang === 'so' ? '🎉 Mahadsanid! Waan kula xiriiri doonaa.' : '🎉 Thanks! We\'ll be in touch.'}
              </motion.p>
            )}
          </motion.div>
        </motion.div>
...
/* ─── Epic Hero SVG (Drag-Interactive) ─── */
  ...
  return (
    ...
    >
      <svg
        ref={svgRef}
        viewBox="0 0 500 500"
        className="w-full h-full drop-shadow-2xl cursor-grab active:cursor-grabbing"
      ...
      >
        ...
        {Array.from({ length: 3 }).map((_, i) => (
          <motion.circle
            key={i}
            cx="250" cy="250" r={40 + i * 30} fill="none" stroke="#00bcd4" strokeWidth="1" opacity="0.3"
            animate={{ rotate: i % 2 ? 360 : -360 }}
            transition={{ duration: 20 + i * 5, repeat: Infinity, ease: 'linear' as const }}
            style={{ transformOrigin: '250px 250px' }}
          />
        ))}
        ...
        <motion.path
          ...
          fill="#ffd700" stroke="#b8860b" strokeWidth="2" className="glow-gold"
        ...
        >
          ...
          <motion.path
            ...
            initial={{ pathLength: 0 }} animate={{ pathLength: [0, 1] }} transition={{ duration: 3, repeat: Infinity }}
          />
        </motion.g>
...
function Hero({ lang }: { lang: string }) {
  ...
  return (
    <section className="min-h-screen bg-gradient-to-br from-navy-900 via-navy-700 to-black relative overflow-hidden">
      ...
      <motion.div
        style={{ y: heroY }}
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 flex items-center justify-center pointer-events-none"
      >
      ...
      >
        ...
        >
          <motion.img
            src="/eazyride-logo.png" alt="EazyRide"
          ...
          <motion.div className="flex flex-wrap justify-center gap-4" variants={stagger} initial="hidden" animate="visible">
            <motion.a href="#downloads" variants={fadeUp}>
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
                className="bg-gradient-to-r from-gold to-copper px-10 py-5 rounded-2xl text-navy-900 font-bold text-lg shadow-2xl glow-gold cursor-pointer">
                <MapPin className="inline w-5 h-5 mr-2 -mt-1" />{t('ctaRide')}
              </motion.button>
            </motion.a>
            <motion.a href="#downloads" variants={fadeUp}>
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
                className="bg-gradient-to-r from-cyan to-gold px-10 py-5 rounded-2xl text-navy-900 font-bold text-lg shadow-2xl glow-cyan cursor-pointer">
                <Smartphone className="inline w-5 h-5 mr-2 -mt-1" />{t('ctaFood')}
              </motion.button>
            </motion.a>
            <motion.a href="#downloads" variants={fadeUp}>
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
                className="bg-gradient-to-r from-copper to-gold px-10 py-5 rounded-2xl text-navy-900 font-bold text-lg shadow-2xl glow-copper cursor-pointer">
          ...
          <motion.p
            ...
            className="mt-8 text-gold text-lg animate-pulse"
          >
            ✨ Drag the SVG to interact!
          </motion.p>
        </motion.div>
      </motion.div>

      <motion.div animate={{ y: [0, 12, 0] }} transition={{ duration: 2, repeat: Infinity }}
        className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30 text-gold text-4xl">↓</motion.div>
    </section>
  );
}
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-12">
          <motion.div initial={{ opacity: 0, x: -60 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8 }}
            ...
            <motion.img src="/eazyride-logo.png" alt="EazyRide" className="w-20 mx-auto mb-6 group-hover:animate-pulse"
              whileHover={{ scale: 1.15, rotate: 360 }} />
            <h3 className="text-3xl font-bold mb-6 text-cyan">{t('services.eazyride')}</h3>
            <ul className="space-y-3 text-lg opacity-90">
              <motion.li whileHover={{ x: 10 }} className="flex items-start"><span className="w-2 h-2 bg-cyan rounded-full mt-2 mr-3 flex-shrink-0" />{t('services.desc1')}</motion.li>
              <motion.li whileHover={{ x: 10 }} className="flex items-start"><span className="w-2 h-2 bg-cyan rounded-full mt-2 mr-3 flex-shrink-0" />{t('services.desc2')}</motion.li>
            </ul>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 60 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8 }}
            whileHover={{ scale: 1.03, y: -8 }}
            className="bg-navy-700/60 backdrop-blur-xl p-10 rounded-3xl border border-gold/30 hover:border-gold/60 shadow-2xl group">
            <img src="/haye-logo.png" alt="Haye!" className="w-20 mx-auto mb-6 group-hover:animate-pulse" />
            <h3 className="text-3xl font-bold mb-6 text-gold">{t('services.haye')}</h3>
            <ul className="space-y-3 text-lg opacity-90">
              <motion.li whileHover={{ x: 10 }} className="flex items-start"><span className="w-2 h-2 bg-gold rounded-full mt-2 mr-3 flex-shrink-0" />{t('services.desc3')}</motion.li>
            </ul>
          </motion.div>
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
          ...
          {roles.map((role, i) => {
            ...
            return (
              <motion.div key={role} initial={{ opacity: 0, y: 50 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                transition={{ delay: i * 0.2 }} whileHover={{ scale: 1.02, y: -10 }}
        ...
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} className="overflow-x-auto">
          <table className="w-full text-left bg-navy-800/50 backdrop-blur-xl rounded-2xl border border-navy-500/50">
            ...
            <tbody>
              {tableRows.map((row, i) => (
                <motion.tr key={i} whileHover={{ backgroundColor: 'rgba(255,215,0,0.1)' }}
...
function Downloads() {
  ...
  const handleDownload = useCallback((file: string) => {
    ...
    confetti({ angle: 90, spread: 55, particleCount: 100, colors: ['#ffd700', '#00bcd4'], shapes: ['star' as any], origin: { y: 0.7 } });
  }, [termsAccepted]);

  return (
    <section ref={ref} id="downloads" className="py-32 px-4 bg-gradient-to-b from-navy-700 to-navy-900 relative">
      <div className="max-w-4xl mx-auto relative z-10">
        <motion.h2 initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} className="text-4xl font-bold text-center mb-10 text-gold">
          {t('downloads.title')}
        </motion.h2>

        {downloadCount > 0 && (
          <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            className="text-center text-gold text-lg mb-4 font-bold">
            ⭐ {downloadCount} download{downloadCount > 1 ? 's' : ''} triggered!
          </motion.p>
        )}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.2 }}
          ...
          <label className="flex items-start gap-3 cursor-pointer group text-white text-base max-w-xl">
            <input type="checkbox" checked={termsAccepted}
              onChange={(e) => { setTermsAccepted(e.target.checked); if (e.target.checked) setShowWarning(false); }}
            ...
            <span>
              ...
              <button type="button" onClick={() => setShowTerms(true)}
                ...
                <FileText className="inline w-4 h-4 mr-1 -mt-0.5" />{t('terms.link')}
              </button>
            </span>
          </label>
          {showWarning && (
            <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
              className="text-red-400 text-sm font-semibold text-center">⚠️ {t('terms.mustAgree')}</motion.p>
          )}
        </motion.div>

        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {apks.map((apk, i) => (
            <motion.button key={i} whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.97 }}
    ...
    <div className="font-sans antialiased">
      ...
      <nav className="fixed top-0 w-full bg-navy-900/95 backdrop-blur-xl z-50 px-4 py-4 border-b border-navy-500/50">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <motion.img src="/eazyride-logo.png" alt="EazyRide" className="w-14 cursor-pointer"
          ...
          <motion.div className="flex space-x-2 rtl:space-x-reverse">
            ...
            <button onClick={() => handleLangSwitch('so')}
            ...
            <button onClick={() => handleLangSwitch('ar')}
