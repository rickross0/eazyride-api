...
exports.getReports = async (req, res, next) => {
  try {
    const [revenue, userGrowth, rideStats] = await Promise.all([
      exports.getRevenueReport(req, res, next).catch(() => null),
      exports.getUserGrowthReport(req, res, next).catch(() => null),
      exports.getRideStatsReport(req, res, next).catch(() => null),
    ]);
    res.json({ success: true, data: { revenue, userGrowth, rideStats } });
  } catch (err) {
    next(err);
  }
};
...
exports.generateReport = async (req, res, next) => {
  try {
    const { type } = req.body;
    if (type === 'revenue') return exports.getRevenueReport(req, res, next);
    if (type === 'userGrowth') return exports.getUserGrowthReport(req, res, next);
    if (type === 'rideStats') return exports.getRideStatsReport(req, res, next);
