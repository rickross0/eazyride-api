...
exports.register = async (req, res, next) => {
  try {
    const { phone, email, password, firstName, lastName, role = 'RIDER' } = req.body;
    const validRoles = ['RIDER', 'DRIVER', 'STORE_OWNER', 'SERVICE_PROVIDER'];
    ...
    if (userRole === 'RIDER') {
      ...
      await prisma.driverProfile.create({
        data: {
          ...
          vehiclePlate: req.body.vehiclePlate || 'SL-0000',
          status: 'PENDING'
        }
      });
    } else if (userRole === 'STORE_OWNER') {
      await prisma.storeOwnerProfile.create({ data: { userId: user.id } });
    } else if (userRole === 'SERVICE_PROVIDER') {
      await prisma.providerProfile.create({
        data: {
          userId: user.id,
          businessName: req.body.businessName || firstName + "'s Business",
          category: req.body.category || 'CAR_RENTAL',
          phone,
          isApproved: false,
          commissionRate: 5
        }
